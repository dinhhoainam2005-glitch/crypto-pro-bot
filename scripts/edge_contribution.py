"""
RETURN CONTRIBUTION - TOP EDGES LÃI & LỖ
"""
...
print("="*70)
print("📊 TOP 10 EDGES ĐÓNG GÓP LỢI NHUẬN & GÂY DRAWDOWN")
print("="*70)
...
for tf in ['15m','1h','4h','1d']:
    for coin in COINS:
        ...
        for cond_str, direction in ALL_EDGES[tf][coin]:
            ...
            total_pnl = valid.sum() * 100
            edge_pnl.append({
                'edge': f"{coin} {tf} {direction} {cond_str}",
                'n': len(valid), 'total_pnl': total_pnl,
                'avg_ret': valid.mean()*100, 'wr': (valid>0).sum()/len(valid)*100
            })

df_pnl = pd.DataFrame(edge_pnl).sort_values('total_pnl', ascending=False)

print(f"\n🏆 TOP 10 LỢI NHUẬN:")
print(f"{'Edge':<50}{'n':>8}{'Total PnL%':>12}{'Avg Ret%':>10}{'WR%':>8}")
print("-"*90)
for _, r in df_pnl.head(10).iterrows():
    print(f"{r['edge']:<50}{r['n']:>8}{r['total_pnl']:>+11.1f}{r['avg_ret']:>+9.2f}{r['wr']:>7.0f}%")

print(f"\n🔻 TOP 10 GÂY DRAWDOWN:")
print(f"{'Edge':<50}{'n':>8}{'Total PnL%':>12}{'Avg Ret%':>10}{'WR%':>8}")
print("-"*90)
for _, r in df_pnl.tail(10).iterrows():
    print(f"{r['edge']:<50}{r['n']:>8}{r['total_pnl']:>+11.1f}{r['avg_ret']:>+9.2f}{r['wr']:>7.0f}%")
