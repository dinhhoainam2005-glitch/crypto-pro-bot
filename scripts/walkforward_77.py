"""
WALK-FORWARD 77 EDGES - 4 FOLDS (2018-2026)
"""
...
def get_events_for_period(df, coin, tf, horizon, start, end):
    events = []
...
def simulate(events, start, end):
    ...
    return eqs.iloc[-1], dd, sh

FOLDS = [
    ('2020-09-01','2022-06-30','2022-07-01','2023-06-30'),
    ('2021-01-01','2023-06-30','2023-07-01','2024-06-30'),
    ('2022-01-01','2024-06-30','2024-07-01','2025-06-30'),
    ('2023-01-01','2025-06-30','2025-07-01','2026-06-22'),
]

print("="*60)
print("🧪 WALK-FORWARD 77 EDGES - 4 FOLDS")
print("="*60)
...
for i, (is_s, is_e, oos_s, oos_e) in enumerate(FOLDS):
    ...
    for tf in ['15m','1h','4h','1d']:
        for coin in COINS:
            ...
            is_events += get_events_for_period(df, coin, tf, HORIZONS[tf], is_s, is_e)
    ...
    for tf in ['15m','1h','4h','1d']:
        for coin in COINS:
            df = load_coin_data(coin, tf)
            if df is None: continue
            oos_events += get_events_for_period(df, coin, tf, HORIZONS[tf], oos_s, oos_e)
    
    fc, dd, sh = simulate(oos_events, oos_s, oos_e)
    print(f"Fold {i+1}: IS={is_s[:4]}-{is_e[:4]} → OOS={oos_s[:4]}-{oos_e[:4]}")
    print(f"   Return={((fc/INITIAL_CAPITAL)-1)*100:+.1f}% | DD={dd:.1f}% | Sharpe={sh:.2f} | Events={len(oos_events)}")
...
for _,_,oos_s,oos_e in FOLDS:
    for tf in ['15m','1h','4h','1d']:
        for coin in COINS:
            df = load_coin_data(coin, tf)
            if df is None: continue
            all_oos += get_events_for_period(df, coin, tf, HORIZONS[tf], '2022-07-01', '2026-06-22')

fc, dd, sh = simulate(all_oos, '2022-07-01', '2026-06-22')
print(f"\n📊 GỘP OOS 2022-2026:")
print(f"Return={((fc/INITIAL_CAPITAL)-1)*100:+.1f}% | DD={dd:.1f}% | Sharpe={sh:.2f} | Events={len(all_oos)}")
print(f"\n🎯 Hoàn thành!")