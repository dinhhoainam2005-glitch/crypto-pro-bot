...
def load_coin_data(coin, tf):
    ...
    oi = os.path.join(OI_DIR, f'{coin}_oi.parquet')
    if not all(os.path.exists(f) for f in [perp, fund, oi]): return None
    df = pd.read_parquet(perp).sort_index()
    fund_df = pd.read_parquet(fund).sort_index()
    oi_df = pd.read_parquet(oi).sort_index()
...
def get_events(df, coin, tf, horizon, start, end):
    ...
    for cond_str, direction in ALL_EDGES[tf][coin]:
        ...
        for et in df.index[mask]:
            # Chỉ lấy tín hiệu khi nến đóng (phút chia hết cho interval)
            if tf == '15m' and et.minute % 15 != 0: continue
            if tf == '1h' and et.minute != 0: continue
            if tf == '4h' and et.hour % 4 != 0: continue
            ...
            events.append({'time': df.index[ei], 'exit_time': df.index[xi],
                          'coin': coin, 'tf': tf, 'direction': direction, 'return': ret})
    return events
    ...
    for d in pd.date_range(start, end, freq='D'):
        ...
        for _, e in df[df['time'].dt.date == d.date()].iterrows():
            ...
            pos.append({'exit_time': e['exit_time'], 'pnl': cap*RISK_PER_TRADE*e['return'],
                       'direction': e['direction'], 'coin': e['coin']})
        eq.append(cap)
...
print("="*60)
print("🧪 BACKTEST 49 EDGES TINH HOA")
print("="*60)
...
for tf in ['15m', '1h', '4h', '1d']:
    for coin in COINS:
        ...
        all_events += get_events(df, coin, tf, HORIZONS[tf], '2020-09-01', '2026-06-22')

fc, dd, sh = simulate(all_events, '2020-09-01', '2026-06-22')
print(f"\n📊 KẾT QUẢ:")
